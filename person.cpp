...
/// CLEAN UP
// 1. clean up all vairblaes not needed for HPV and HPV screening
// 2. Remove hard code for ncds and cancers
...
person::person()											// First 'person' class second constructor/variable and no return type means its a constructor
{
    ...
    HPV[5]={-999};
    HPV_Status[5]={-999};
    HPV_ReTest_date[5]={-999};
    HPV_DateofRecovery[5]={-999};
    HPV_StatusAtRecovery[5]={-999};
    HPV_DateofProgression[5]={-999};
    HPV_DateOfCIS=-999;
    VIAcount=0;
    VIA=-999;
    ...
    CVD_First_Screening_Date=-999;
    CVD_First_Screening=0;
...
void person::GetDateOfBaby(){								// This method already calculates the child's month of birth by providing a year of birth with decimal
    ...
    if (Sex == 2 && Alive == 1 && Age<maxAgeBirth && AgeAtDeath>=minAgeBirth){
        ...
        int index=DoB-1901+minAgeBirth;						// index for large arrays, related to year the patient will be 15 and ready for birth
        int NrChildren = 0;
        ...
        while (m<NrChildren){
            ...
            DateOfBirthTest = DoB + minAgeBirth +j;
...
void person::GetMyDateOfHPVInfection(){
    ...
    if(Sex==2)
    {
        ...
        if (h>HPVarray[1][65]*HPV_Array_buffer){HPV[HPVcount]=-988;              // -988 in case they do NOT get HPV ever
        }
        
        if (h<=HPVarray[1][65]*HPV_Array_buffer){                                      // In case they DO get HPV in their life
